 ...
 */
public class ChangePassageApp extends Activity{
            ...
            public void onClick(View v) {
               ...
               String password_old=changeFirstText.getText().toString().trim();
                ...
                if(TextUtils.isEmpty(password_new)||TextUtils.isEmpty(password_confirm_new)||TextUtils.isEmpty(password_old)){
                    Toast.makeText(ChangePassageApp.this, "输入密码为空", Toast.LENGTH_SHORT).show();
                    return;
                }
                String lockapppassword=sp.getString("applockpassword2", "");
                if(password_new.equals(password_confirm_new)&&MD5Utils.md5Encode(password_old).equals(lockapppassword)){
                    SharedPreferences.Editor editor=sp.edit();
                    editor.putString("applockpassword2", MD5Utils.md5Encode(password_new));
                    editor.commit();

                    boolean b = NetworkUtil.checkedNetWork(ChangePassageApp.this);
                    //判断网络连接情况
                    if (b){
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                //网络正常则获取密码初始化接口
                                RestTemplateUtil.getPwd(application.getUrl_get_app_pwd()+"/"+ MacUtil.getMacAddress()+"/"+password_new+"/"+1);
                            }
                        }).start();
                    }

                    Toast.makeText(ChangePassageApp.this, "密码修改成功", Toast.LENGTH_SHORT).show();
                    finish();

                }
                else if(!MD5Utils.md5Encode(password_old).equals(lockapppassword)&&password_new.equals(password_confirm_new))
                {
                    Toast.makeText(ChangePassageApp.this, "原密码错误", Toast.LENGTH_SHORT).show();
                }

                else{
                    Toast.makeText(ChangePassageApp.this, "两次密码输入不一致", Toast.LENGTH_SHORT).show();
